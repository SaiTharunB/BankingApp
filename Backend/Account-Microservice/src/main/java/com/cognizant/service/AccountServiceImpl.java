...
public class AccountServiceImpl implements AccountService {
	...
	public void statement(TransactionInput transactionInput, String transactionType) {
		...
		Account saccount = getAccount(transactionInput.getSourceAccount());
		Account taccount = getAccount(transactionInput.getTargetAccount());
		if(saccount!=null) {
			...
			if (saccount != taccount) {
				...
				sst.setNarration("Transfer to account number"+ transactionInput.getTargetAccount());
			}
			else {
		...
		if(saccount != taccount) {
			...
			tst.setWithdraws(0);
			tst.setNarration("Transferd from account number"+ transactionInput.getSourceAccount());
			tst.setRefNo(0);
	...
	@Override
	public Account updateTargetTransfer(long targetAccount, double amount) {
		
		Account tAccount= accountRepository.findByAccountNumber(targetAccount);
		
		
		tAccount.setBalance(tAccount.getBalance()+amount);
		
		return accountRepository.save(tAccount);
		
	}

	@Override
	public Account updateSourceTransfer(long sourceAccount, double amount) {
		Account sAccount = accountRepository.findByAccountNumber(sourceAccount);
		sAccount.setBalance(sAccount.getBalance()-amount);
		return accountRepository.save(sAccount);
